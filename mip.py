...
# base_points - list of points from the base that are within delta to given points
# base_points_mapping - maps base_points index to points index
...
def filterPoints(base, points, delta):
    ...
    return points_mapping, base_points, new_points, new_points_mapping
...
def makeDoc(sumgen, groupdict, edges):
    groupnum = dict()
    groups = []
    i = 0
...
def buildMip(doc, target):
    ...
    m.setObjective(quicksum(pv[i] for i in range(len(doc["points"]))), "minimize")
    m.optimize()
    print("Input")
    print(target)
    optimized = []
...
def makeRecipe(doc, base_len, point_list, target_list, delta):
...
def main():
    ...
    points_mapping, base_points, new_points, new_points_mapping = filterPoints(sumgen, target, MAX_DIST)
    ...
    print("Making doc.")
    doc = makeDoc(sumgen, groupdict, edges)
    with open('graph.json', 'w') as outfile:
       json.dump(doc, outfile, separators=(',',':'))

    print("Building MIP.")
    optimized = buildMip(doc, points_mapping)
    ...
    recipe = makeRecipe(doc, len(base), optimized, points_mapping, MAX_DIST)
    #print(recipe)
    out = normailzeRecipe(recipe, len(base), input["e"], points_mapping)
